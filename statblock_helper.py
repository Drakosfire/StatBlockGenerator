...
def gen_file_name():
  ...
  input_dir = f"/app/output/{u.make_folder()}" 
...
def generate_statblock(input):
    ...
    model_directory = "/app/models/Speechless-Llama2-Hermes-Orca-Platypus-WizardLM-13B-GPTQ"
    ...
    lora_directory = "/app/models/statblock-alpha"
    ...
    input_context = f"Write a homebrewery formatted dungeons and dragons statblock of {generate_statblock.input} any abilities that reference actions need to have those actions defined \n" + statblock_start
    print(input_context)
...
def md_process(input_md,output_name):
  ...
  # Docker compused from this https://github.com/G-Ambatte/homebrewery/tree/experimentalCommandLineBrewProcess
  process_call = f"node /app/homebrewery-cli/cli/process.js --input {abs_path} --output {input_dir}/{file_name}.html --renderer v3 --overwrite"
  print(process_call) 
