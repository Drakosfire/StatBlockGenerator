...
def make_folder():
    ...
    folder_path = f"/app/output/{foldertimestr}"
...
def make_image_name():
    ...
    image_name = f"/app/output/{make_folder()}/{dsh.generate_monster_desc.monster_type}{timestr}.png"
...
def make_user_log() :
    del user_log[:]
    timestr = time.strftime("%Y%m%d-%H%M%S")
    folder_path = f"/app/output/{make_folder()}"
