...
def generate_monster_desc(monster_type, monster_color,monster_size,monster_loc):
    ...
    del_sd_input()
    print(sd_input)
    prompt_template = f"You are an intelligent, accomplished, expert writer of fantasy fiction that uses a diverse and wide ranging vocabulary with a particular knack for painting mental images. Do not write instructions or tips, only write only a concise, visually descriptive, detailed and colorful three to 5 sentences about the appearance of the {monster_type}, include description of its body and all of it's parts. It has these qualities {monster_color} is size {monster_size} and can be found {monster_loc}. The focus should be on describing it's appearance. Always start with the creatures name/type : {monster_type}"
