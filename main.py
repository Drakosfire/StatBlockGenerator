...
# This global is sloppy and is a sign it's time to refactor
...
def gen_mon_statblock(challenge_rating,abilities):     
    input = dsh.sd_input[0] + f"please build it around challenge rating {challenge_rating}, create and give it the abilities {abilities}"
...
with gr.Blocks() as demo:
    ...
    clear_cache = gr.Button(value = "Clear Cache")
    clear_cache.click(u.clear_cache)
    with gr.Tab("Generator"):
        with gr.Row():
            ...
            with gr.Column(scale = 1):
                ...
                mon_desc = gr.Textbox(label = 'Monster Description Editable', lines = 16, interactive=True)
        ...
        image_Gen = gr.Button(value = "Generate 4x Images about 50s" ) 
    ...
    with gr.Tab("Statblock"):
        ...
        gr.Interface(
            ...
            outputs = gr.Textbox(label = "Statblock Editable", lines = 20, interactive=True, elem_id= "User Input"),
...
# Implementing malloc trim to release memory being held after generation and leading to a locked up cpu with no RAM
def trim_memory() -> int:
...
def main() -> None:
  # run web server, expose port 8000, share to create web link, give app access folder path
  if __name__ == '__main__':
    demo.launch(server_name = "0.0.0.0", server_port = 8000, share = False, allowed_paths = ["/app"])
